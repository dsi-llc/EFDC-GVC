...
C 
     ...
     &  CALL CALTRAN (ISTL,IS2TL,1,1,SAL,SAL1)
      IF(ISTRAN(2).EQ.1.AND.ISCDCA(2).LT.4)
     &  CALL CALTRAN (ISTL,IS2TL,2,2,TEM,TEM1)
      IF(ISTRAN(3).EQ.1.AND.ISCDCA(3).LT.4)
     &  CALL CALTRAN (ISTL,IS2TL,3,3,DYE,DYE1)
C     IF(ISTRAN(3).EQ.1) CALL CALTRAN (ISTL,IS2TL,4,4,SFL,SFL1)
...
C  SUBROUTINE CALTRAN (ISTL,IS2TL,MVAR,M,CON,CON1)
C 
         CALL CALTRAN (ISTL,IS2TL,5,M,TVAR2S,TVAR1S)
         DO K=1,KC
         DO L=1,LC
         ...
         ENDDO
C
        IF(NT.EQ.1.AND.ISTOXALL.EQ.1)THEN
          DO K=1,KC
          DO L=2,LA
            TMPFLX=DELT*DZC(K)*UHDY2(L,K)*TOX1(LUPU(L,K),K,1)
            TMPFLY=DELT*DZC(K)*VHDX2(L,K)*TOX1(LUPV(L,K),K,1)
            DO NS=1,NSED+NSND+2
              ATOXPFWX(L,NS,1)=ATOXPFWX(L,NS,1)
     &                        +TMPFLX*TOXPFW(LUPU(L,K),K,NS,1)
                ATOXPFWY(L,NS,1)=ATOXPFWY(L,NS,1)
     &                        +TMPFLY*TOXPFW(LUPV(L,K),K,NS,1)
            ENDDO
            DO NS=1,NSED
              ASEDFWX(L,NS)=ASEDFWX(L,NS)
     &                     +DELT*DZC(K)*UHDY2(L,K)*SED1(LUPU(L,K),K,NS)
                ASEDFWY(L,NS)=ASEDFWY(L,NS)
     &                     +DELT*DZC(K)*VHDX2(L,K)*SED1(LUPV(L,K),K,NS)
            ENDDO
            DO NX=1,NSND
              NS=NX+NSED
              ASEDFWX(L,NS)=ASEDFWX(L,NS)
     &                     +DELT*DZC(K)*UHDY2(L,K)*SND1(LUPU(L,K),K,NX)
                ASEDFWY(L,NS)=ASEDFWY(L,NS)
     &                     +DELT*DZC(K)*VHDX2(L,K)*SND1(LUPV(L,K),K,NX)
              ASEDFBLX(L,NX)=ASEDFBLX(L,NX)+DELT*QSBDLDX(L,NX)
                ASEDFBLY(L,NX)=ASEDFBLY(L,NX)+DELT*QSBDLDY(L,NX)
            ENDDO
            ATOXFDFWX(L,1)=ATOXFDFWX(L,1)
     &                        +TMPFLX*TOXFDFW(LUPU(L,K),K,1)
              ATOXFDFWY(L,1)=ATOXFDFWY(L,1)
     &                        +TMPFLY*TOXFDFW(LUPV(L,K),K,1)
            ATOXCDFWX(L,1)=ATOXCDFWX(L,1)
     &                        +TMPFLX*TOXCDFW(LUPU(L,K),K,1)
              ATOXCDFWY(L,1)=ATOXCDFWY(L,1)
     &                        +TMPFLY*TOXCDFW(LUPV(L,K),K,1)
            ATOXSOUR(L,1)=ATOXSOUR(L,1)+DELT*MAX(FQC(L,K),0.0)
              ATOXSINK(L,1)=ATOXSINK(L,1)+DELT*MIN(FQC(L,K),0.0)
          ENDDO
          ENDDO
        ENDIF
C
...
C  SUBROUTINE CALTRAN (ISTL,IS2TL,MVAR,M,CON,CON1)
C 
         CALL CALTRAN (ISTL,IS2TL,6,M,TVAR2S,TVAR1S)
         DO K=1,KC
         DO L=1,LC
...
C  SUBROUTINE CALTRAN (ISTL,IS2TL,MVAR,M,CON,CON1)
C 
         CALL CALTRAN (ISTL,IS2TL,7,M,TVAR2S,TVAR1S)
         DO K=1,KC
         DO L=1,LC
...
C
      ...
      IF(ISTRAN(6).EQ.1.AND.ISCDCA(6).LT.4)THEN
         ...
         DO K=1,KC
         DO L=1,LC
          TVAR1S(L,K)=SEDFLOCDIA(L,K)
          TVAR2S(L,K)=SEDFLOCDIA(L,K)
         ENDDO
         ENDDO
         CALL CALTRAN (ISTL,IS2TL,9,M,TVAR2S,TVAR1S)
         DO K=1,KC
         DO L=1,LC
          SEDFLOCDIA(L,K)=TVAR1S(L,K)
          SEDFLOCDIA(L,K)=TVAR2S(L,K)
         ENDDO
         ENDDO
      ENDIF
      ENDIF
...
C
      ...
      DO ND=1,NDM
       ...
       IF(ISTRAN(6).GE.1)THEN
         ...
         ENDDO
       ENDIF
       IF(ISTRAN(6).GE.1.AND.ISEDVW.EQ.99)THEN
          DO L=LF,LL
           SEDFLOCDIA(L,1)=SEDFLOCDIA(L,1)*EEB(L)
          ENDDO
       ENDIF
...
C
      DO ND=1,NDM
       ...
       DO K=2,KS
        ...
        IF(ISTRAN(6).GE.1.AND.ISEDVW.EQ.99)THEN
           DO L=LF,LL
            SEDFLOCDIA(L,K)=(SEDFLOCDIA(L,K)
     &                      -CCLBTMP(L)*SEDFLOCDIA(L,K-1))*EEB(L)
           ENDDO
        ENDIF
        IF(ISTRAN(7).GE.1)THEN
          DO NS=1,NSND
...
C
      DO ND=1,NDM
       ...
       DO K=KC-1,1,-1
        ...
        IF(ISTRAN(6).GE.1.AND.ISEDVW.EQ.99)THEN
           DO L=LF,LL
            SEDFLOCDIA(L,K)=SEDFLOCDIA(L,K)-CU1(L,K)*SEDFLOCDIA(L,K+1)
           ENDDO
        ENDIF
        IF(ISTRAN(7).GE.1)THEN
          DO NS=1,NSND
...
C
        ...
        CONASMOLD=SAL(L,K)
        IDIRASM=1
        IF(IDIRCDA(NLC).EQ.1.AND.UHDY2(L,K).LT.0.0)IDIRASM=0
        IF(IDIRCDA(NLC).EQ.-1.AND.UHDY2(L,K).GE.0.0)IDIRASM=0
        IF(IDIRCDA(NLC).EQ.2.AND.VHDX2(L,K).LT.0.0)IDIRASM=0
        IF(IDIRCDA(NLC).EQ.-2.AND.VHDX2(L,K).GE.0.0)IDIRASM=0
        NSID=NCSERA(NLC,1)
        IF(IWASM.EQ.1) WRITE(1,1111)N,NLC,ICDA(NLC),JCDA(NLC),NSID,
     &                 CSERT(K,NSID,1),SAL(L,K)
        IF(ITPCDA(NLC).EQ.0)THEN
          IF(NSID.GT.0)THEN
          IF(CSERT(K,NSID,1).GT.0.0.AND.IDIRASM.EQ.1)THEN
...
C
     ...
     &                 CSERT(K,NSID,2),TEM(L,K)
        IF(ITPCDA(NLC).EQ.0)THEN
          IF(NSID.GT.0)THEN
          IF(CSERT(K,NSID,2).GT.0.0.AND.IDIRASM.EQ.1)THEN
...
C
        ...
        IF(ITPCDA(NLC).EQ.0)THEN
          IF(NS.GT.0)THEN
          IF(CSERT(K,NSID,3).GT.0.0.AND.IDIRASM.EQ.1)THEN
...
C
        ...
        IF(ITPCDA(NLC).EQ.0)THEN
          IF(NSID.GT.0)THEN
          IF(CSERT(K,NSID,4).GT.0.0.AND.IDIRASM.EQ.1)THEN
...
C
        ...
        IF(ITPCDA(NLC).EQ.0)THEN
          IF(NSID.GT.0)THEN
          IF(CSERT(K,NSID,M).GT.0.0.AND.IDIRASM.EQ.1)THEN
