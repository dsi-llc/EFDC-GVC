...
C**********************************************************************C
C
C ** SUBROUTINE CALPUVD CALCULATES THE EXTERNAL SOLUTION FOR P, UHDYE,
...
C**********************************************************************C
C
C **  STORE NEW DEPTHS IN FP ARRAY 
C
C----------------------------------------------------------------------C
C
      IF(ISTL.EQ.3)THEN
...
C
      DO L=2,LA
      LS=LSC(L)      
      TVAR3E(L)=SUB(L)*FUHDYE(L)
      TVAR3N(L)=SVB(L)*FVHDXE(L)
      ENDDO
C
C**********************************************************************C
C
C **  APPLY WEAK TIME FILTER FOR 3TL
C
C----------------------------------------------------------------------C
C
      IF(ISTL.EQ.3)THEN
      DO L=2,LA
       UHDYE(L)=UHDYE(L)+FILT3TL*(UHDY1E(L)-2.*UHDYE(L)+TVAR3E(L)) 
       VHDXE(L)=VHDXE(L)+FILT3TL*(VHDX1E(L)-2.*VHDXE(L)+TVAR3N(L)) 
       HP(L)=HP(L)+FILT3TL*(H1P(L)-2.*HP(L)+FP(L))
      ENDDO
      ENDIF 
C
C**********************************************************************C
...
C      LN=LNC(L)
C      FUHDYE(L)=UHDY1E(L)
C     &   -DELTD2*SUB(L)*HRUO(L)*HUTMP(L)*(P(L)-P(L-1)+P1(L)-P1(L-1))
C     &         +SUB(L)*DELT*DXIU(L)*(DXYU(L)*(TSX1(L)-TBX1(L))
C     &         +FCAXE(L)+FPGXE(L)-SNLT*FXE(L))
C      FVHDXE(L)=VHDX1E(L)
C     &   -DELTD2*SVB(L)*HRVO(L)*HVTMP(L)*(P(L)-P(LS )+P1(L)-P1(LS ))
C     &         +SVB(L)*DELT*DYIV(L)*(DXYV(L)*(TSY1(L)-TBY1(L))
C     &         -FCAYE(L)+FPGYE(L)-SNLT*FYE(L))
C      UTMP=SUB(L)*FUHDYE(L)
C      VTMP=SVB(L)*FVHDXE(L)
C      UHDYE(L)=UTMP
C      VHDXE(L)=VTMP
C      UHE(L)=UTMP*DYIU(L)
C      VHE(L)=VTMP*DXIV(L)
