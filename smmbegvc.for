...
C
      DO L=2,LA
        IZ = ISMZMAP(L)
        SMT(L) = (SMT(L) + SM1DIFT(IZ)*TEM(L,KGVCP(L))) * SM2DIFT(IZ)
...
C MRM +++++++++ ADDED BY M. MORTON 07/24/98
          ...
          WRITE(1,911) TIMTMP, L, IL(L), JL(L), TEM(L,KGVCP(L)), SMT(L)
911       FORMAT(/,'ERROR!! INVALID SEDIMENT TEMPERATURE',/,
     +     'TIME, L, I, J, TEM(L,KGVCP(L)), SMT(L) = ',
     +         F10.5, 3I4, 2F10.4)
          CLOSE(1)
C MRM +++++++++ ADDED BY M. MORTON 07/24/98
          PRINT*, 'L, TEM(L,KGVCP(L)), SMT(L) = ',
     +      L,TEM(L,KGVCP(L)),SMT(L)
C
...
CMRM D.O. EVENTS:
CMRM     XSMO20(L) = MAX( WQV(L,KGVCP(L),19), 0.1 )
        XSMO20(L) = MAX( WQV(L,KGVCP(L),19), 3.0 )
...
C
        ...
        SMJGCH4(L) = GJCH4SM
C
        WQBFNH4(L) = SMSS(L) * (SMFD1NH4*SM1NH4(L) - WQV(L,KGVCP(L),14))
        WQBFNO3(L) = SMSS(L) * (SM1NO3(L) - WQV(L,KGVCP(L),15))
        WQBFCOD(L) = SMJAQH2S(L) - SMSS(L)*WQV(L,KGVCP(L),18)
C       WQBFCOD(L) = SMJAQH2S(L)
C        WQBFCOD(L) = AQJH2SSM-SMSS(L)*WQV(L,KGVCP(L),18) + AQJCH4SM
C
      ENDIF
      ENDDO
C
...
C
     ...
     *    + SMW2(ISMZMAP(L))
        ...
        B11PO4SM = SMSS(L) * WQPO4D(L,KGVCP(L))
     ...
     *    B11PO4SM,B22PO4SM)
        ...
        WQBFPO4D(L) = SMSS(L) * (SMD1PO4(L) - WQPO4D(L,KGVCP(L)))
        SM1PO4(L) = RSM1PO4
        SM2PO4(L) = RSM2PO4
      ENDIF
      ENDDO
