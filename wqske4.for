...
C
       DO L=LF,LL
        ...
        WQGNC = RNH4NO3 / (WQKHNC+RNH4NO3+ 1.E-18)
        WQGND = RNH4NO3 / (WQKHND+RNH4NO3+ 1.E-18)
        WQGNG = RNH4NO3 / (WQKHNG+RNH4NO3+ 1.E-18)
        WQGPC = PO4DWQ / (WQKHPC+PO4DWQ+ 1.E-18)
        WQGPD = PO4DWQ / (WQKHPD+PO4DWQ+ 1.E-18)
        WQGPG = PO4DWQ / (WQKHPG+PO4DWQ+ 1.E-18)
...
C
        WQKDOC = (WQKDC(IZ) + WQKDCALG*WQOBTOT + XMRM)*WQTDMNL(IWQT(L))
...
C 7-10 PHOSPHORUS
C
        WQAPC(L) = 1.0 / (WQCP1PRM + WQCP2PRM*EXP(-WQCP3PRM*PO4DWQ))
        WQKHP = (WQKHPC+WQKHPD+WQKHPG) / 3.0
        WQTT1 = WQKHP / (WQKHP+PO4DWQ+ 1.E-18) * WQOBTOT
...
C
        WQO18(L)= -DTWQO2*WQKCOD(IWQT(L),IZ)*O2WQ /
     +             (WQKHCOD(IZ) + O2WQ + 1.E-18)
C
...
CTT UPDATE MODIFIED NEXT LINE ADDED LINE BELOW COMMENTED OUT
        WQV(L,K,17)=SCB(L)*( WQRR(L)*WQKK(L) )+(1.-SCB(L))*WQVO(L,K,17)
...
C
         IF(IDNOTRVA.GT.0.AND.K.EQ.1)THEN
           ...
           XMRM = (1.0-WQFCDM)*O2WQ*WQBMM(L)/(WQKHRM(IZ)+O2WQ+ 1.E-18)
...
C
         DO L=LF,LL
CTT UPDATE MODIFIED NEXT LINE ADDED LINE BELOW COMMENTED OUT
        WQV(L,K,20)=SCB(L)*( WQRR(L)*WQKK(L) )+(1.-SCB(L))*WQVO(L,K,20)
...
CTT UPDATE MODIFIED NEXT LINE ADDED LINE BELOW COMMENTED OUT
        WQV(L,K,21)=SCB(L)*( WQRR(L)*WQKK(L) )+(1.-SCB(L))*WQVO(L,K,21)
