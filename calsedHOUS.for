...
C          IF(N.GT.ISEDDT)THEN
            DO L=2,LA
              IF(SED(L,K+1,NS).GT.0.0)THEN
                FLOCDIA(L,K+1)=SEDFLOCDIA(L,K+1)/SED(L,K+1,NS)
              ELSE
                FLOCDIA(L,K+1)=0.001
              ENDIF
            ENDDO
...
C CONVERT SED CONCENTRATION FROM MG/L TO GM/GM**3
              ...
              IF(USTARTMP.GT.0.0.AND.SEDTMP.GT.0.0)THEN
                TMP=1.+0.4*QCELLCTR(L)/USTARTMP
                SHEAR=2.5*USTARTMP*TMP*HPI(L)
                SEDSHEAR=SHEAR*SEDTMP
                FDIATMP=FLOCDIA(L,K+1)
  999           CONTINUE
                  TERM1=300.0*( 3.*SQRT(SEDSHEAR)*(FDIATMP**1.2)
     &              +SEDTMP*(FDIATMP**0.2) )
                  TERM2=1.-4.6E5*( 3.5*SEDSHEAR*(FDIATMP**2.18)
     &              +SEDTMP*(FDIATMP**1.18) )
                  FDIATMPN=FDIATMP+DELT*TERM1*TERM2
                  ERR=ABS(FDIATMPN-FDIATMP)
                  FDIATMP=FDIATMPN
                  IF(ERR.GT.1.e-5)GO TO 999
                FLOCDIA(L,K+1)=FDIATMP  
                SEDFLOCDIA(L,K+1)=FDIATMP*SED(L,K+1,NS)
                WSETINF=80.
C CONVERT FLOC DIAMETER TO UM 
...
cSO 05/14/04
                  IF ( L.LE.2042 ) THEN
                    WESE=TMPSEDHID*WESE*(TAUE**EXPO_DS) ! All of 5C
                  ELSE                                !      + Woods Pond
                    WESE=TMPSEDHID*WESE*(TAUE**EXPO_US ) ! North of (All of
...
cjmh216     &                        +S2TL*SEDB1(L,KBT(L),NS)
            ...
            IF(IBMECH.EQ.1.AND.SEDVRDT.LT.0.0)THEN
              QSBDTOP(L)=QSBDTOP(L)+0.001*SEDF(L,0,NS)/SDENAVG(L,KBT(L))
              QWBDTOP(L)=QWBDTOP(L)
     &          +0.001*VDRBED(L,KBT(L))*SEDF(L,0,NS)/SDENAVG(L,KBT(L))
            ELSE
        ...
        DO NS=1,NSED
          DO L=2,LA
            IF(SEDB(L,KBT(L),NS).LT.0.)THEN
              WRITE(1,108)N,NS,IL(L),JL(L),KBT(L),SEDB(L,KBT(L),NS),
     &             SEDF(L,0,NS)
            ENDIF
          ENDDO
        ENDDO
