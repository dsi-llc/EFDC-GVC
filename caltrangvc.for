...
C**********************************************************************C
C
      SUBROUTINE CALTRANGVC(ISTL,IS2TL,MVAR,M,CON,CON1)
C
...
C	ENDIF
c
C  811 FORMAT('CK ADV GVC ',5I6,8E14.6)
C----------------------------------------------------------------------C
C
...
C
      ...
      RDZIG=DZIG(K)
      DO L=2,LA
      WWW(L,K)=W2(L,K)*(CON(L,K+1)-CON(L,K))*GVCSCLPI(L)*HPI(L)*RDZIG
      ENDDO
      ENDDO
C
C----------------------------------------------------------------------C
...
C
      IF(UHDY2(L,K).GE.0.0)THEN
        UTERM=UTERM-0.5*DELTA*GVCSCLU(L)*UHDY2(L,K)*
...
C
      IF(VHDX2(L,K).GE.0.0)THEN
        VTERM=VTERM-0.5*DELTA*GVCSCLV(L)*VHDX2(L,K)*
...
C
	...
	IF(LGVCP(L,K))THEN
      ...
      CWMIN=SUB3D(L,K)*CONTMN(L-1,K)+1.E+6*(1.-SUB3D(L,K))
      CEMIN=SUB3D(L+1,K)*CONTMN(L+1,K)+1.E+6*(1.-SUB3D(L+1,K))
      CSMIN=SVB3D(L,K)*CONTMN(LS,K)+1.E+6*(1.-SVB3D(L,K))
      CNMIN=SVB3D(LN,K)*CONTMN(LN,K)+1.E+6*(1.-SVB3D(LN,K))
...
C	ENDIF
c
C  801 FORMAT('FLUX LIM 1 GVC ',4I6,8E14.6)
C
C **  LIMIT FLUXES
C
      DO K=1,KC
...
C        DO K=1,KC
C        DO L=2,LA
C	  WRITE(8,802)N,IL(L),JL(L),K,CON(L,K),CH(L,K),
C     &                              FUHU(L,K),DU(L,K),DV(L,K)
C        ENDDO
C        ENDDO  
C	ENDIF
c
C  802 FORMAT('FLUX LIM 2 GVC ',4I6,8E14.6)
C
c  GVC DIAGNOSTIC
C
...
C
C NOT MODIFIED FOR GVC BEYOND THIS POINT SINCE GVC DOES
C NOT CURRENTLY SUPPORT DRYING AND WETTING - JMH 08/04/04
C
C**********************************************************************C
C**********************************************************************C
C
...
C
	...
	IF(LMASKDRY(L))THEN
        LN=LNC(L)
        AWW=ABS(W2(L,K))
        WTERM=AWW*(1.-DELTA*AWW*DZIG(K)*HPI(L))
     *           *(CON(L,K+1)-CON(L,K))
