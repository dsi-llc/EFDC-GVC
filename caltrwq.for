...
C
      ...
      DO L=2,LA
      LS=LSC(L)
      WWW(L,K)=WWQ(L,K)*(CON(L,K+1)-CON(L,K))*RDZIG/HWQ(L)
      ENDDO
      ENDDO
C
C----------------------------------------------------------------------C
C
      ...
      AUHU=ABS(UHDYWQ(L,K))
      AVHV=ABS(VHDXWQ(L,K))
C      UTERM=AUHU*(1.-DELTA*AUHU/(DXYU(L)*HU(L)))*(CON(L,K)-CON(L-1,K))
C      VTERM=AVHV*(1.-DELTA*AVHV/(DXYV(L)*HV(L)))*(CON(L,K)-CON(LS,K))
      UTERM=AUHU*(CON(L,K)-CON(L-1,K))
      VTERM=AVHV*(CON(L,K)-CON(LS,K))
      IF(ISADAC(MVAR).GE.2)THEN
        SSCORUE=DELTA*RDZIC*DXYIP(L  )*HWQI(L  )*(FQCPAD(L  ,K)
     &                                 -QSUMPAD(L  ,K)*CON(L  ,K))
        SSCORUW=DELTA*RDZIC*DXYIP(L-1)*HWQI(L-1)*(FQCPAD(L-1,K)
     &                                 -QSUMPAD(L-1,K)*CON(L-1,K))
        SSCORVN=DELTA*RDZIC*DXYIP(L  )*HWQI(L  )*(FQCPAD(L  ,K)
     &                                 -QSUMPAD(L  ,K)*CON(L  ,K))
        SSCORVS=DELTA*RDZIC*DXYIP(LS )*HWQI(LS )*(FQCPAD(LS ,K)
...
C
      IF(UHDYWQ(L,K).GE.0.0)THEN
        UTERM=UTERM-0.5*DELTA*UHDYWQ(L,K)*
...
C
      IF(VHDXWQ(L,K).GE.0.0)THEN
        VTERM=VTERM-0.5*DELTA*VHDXWQ(L,K)*
...
C
      IF(ISADAC(MVAR).GE.2)THEN
        SSCORWA=DELTA*DZIG(K+1)*HWQI(L)*DXYIP(L)
     &         *(FQCPAD(L,K+1)-QSUMPAD(L,K+1)*CON(L,K+1))
        SSCORWB=DELTA*DZIG(K)*HWQI(L)*DXYIP(L)
...
C
C1031      DO L=2,LA
C1031      CMAX(L,1)=MAX(CONT(L,1),CONT(L,2))
C1031      CMAX(L,KC)=MAX(CONT(L,KS),CONT(L,KC))
C1031      ENDDO
C
C1031      DO K=2,KS
C1031      DO L=2,LA
C1031      CMAX(L,K)=MAX(CONT(L,K-1),CONT(L,K),CONT(L,K+1))
C1031      ENDDO
C1031      ENDDO
C
C1031      DO K=1,KC
...
C1031      LS=LSC(L)
C1031      LN=LNC(L)
C1031      CWMAX=SUB(L)*CONT(L-1,K) 
C1031      CEMAX=SUB(L+1)*CONT(L+1,K)
C1031      CSMAX=SVB(L)*CONT(LS,K)
C1031      CNMAX=SVB(LN)*CONT(LN,K)
C1031      CMAX(L,K)=MAX(CMAX(L,K),CNMAX,CEMAX,CSMAX,CWMAX)
C1031      ENDDO
C1031      ENDDO
C
C1031      DO K=1,KC
C1031      DO L=2,LA
C1031      CONT(L,K)=MIN(CON(L,K),CON2(L,K))
C1031      ENDDO
C1031      ENDDO
C
C1031      DO L=2,LA
C1031      CMIN(L,1)=MIN(CONT(L,1),CONT(L,2))
C1031      CMIN(L,KC)=MIN(CONT(L,KS),CONT(L,KC))
C1031      ENDDO
C
C1031      DO K=2,KS
C1031      DO L=2,LA
C1031      CMIN(L,K)=MIN(CONT(L,K-1),CONT(L,K),CONT(L,K+1))
C1031      ENDDO
C1031      ENDDO
C
C1031      DO K=1,KC
...
C1031      LS=LSC(L)
C1031      LN=LNC(L)
C1031      CWMIN=SUB(L)*CONT(L-1,K)+1.E+6*(1.-SUB(L))
C1031      CEMIN=SUB(L+1)*CONT(L+1,K)+1.E+6*(1.-SUB(L+1))
C1031      CSMIN=SVB(L)*CONT(LS,K)+1.E+6*(1.-SVB(L))
C1031      CNMIN=SVB(LN)*CONT(LN,K)+1.E+6*(1.-SVB(LN))
C1031      CMIN(L,K)=MIN(CMIN(L,K),CNMIN,CEMIN,CSMIN,CWMIN)
C1031      ENDDO
C1031      ENDDO
C
C----------------------------------------------------------------------C
...
C
     ...
     &                               +FVHU(L,K)-FVHV(LN,K))
     &                      +RDZIC*(FWU(L,K-1)-FWV(L,K)) )*HWQI(L)
      DV(L,K)=DELT*SCB(L)*( DXYIP(L)*(FUHU(L+1,K)-FUHV(L,K)
     &                               +FVHU(LN,K)-FVHV(L,K))
     &                      +RDZIC*(FWU(L,K)-FWV(L,K-1)) )*HWQI(L)
      ENDDO
      ENDDO
C
      DO K=1,KC
