...
C**********************************************************************C
C
C ** SUBROUTINE CALPUVF CALCULATES THE EXTERNAL SOLUTION FOR P, UHDYE,
...
C
      ...
      ENDDO
C
      IF(ISTL.EQ.2) GOTO 5000
C
C**********************************************************************C
C
C **  ADD EXTERNAL MODE CORIOLIS CORRECTION IF ISTL=3
C
C**********************************************************************C
C
C **  REVISE EXPLICIT EXTERNAL FUHDYE AND FVHDXE EQUATION TERMS
C
C----------------------------------------------------------------------C
C
      DO L=1,LC
...
C
      DO L=2,LA
       TVAR3E(L)=HUI(L)*UHE(L)
       TVAR3N(L)=HVI(L)*VHE(L)
      ENDDO
C
      DO L=2,LA
       LN=LNC(L)          
       TVAR3C(L)=( FCORC(L)*DXYP(L)
...
C
      DO L=2,LA
       ...
       LNW=LNWC(L)
       LSE=LSEC(L)
       FCAX1E(L)=FCAX1E(L)+0.0625*SCAX(L)*
     &            ( TVAR3C(L)*(TVAR3N(LN)+TVAR3N(L))
     &             +TVAR3C(L-1)*(TVAR3N(LNW)+TVAR3N(L-1)) )
       FCAY1E(L)=FCAY1E(L)+0.0625*SCAY(L)*
     &            ( TVAR3C(L)*(TVAR3E(L+1)+TVAR3E(L))
     &             +TVAR3C(LS)*(TVAR3E(LSE)+TVAR3E(LS)) )
      ENDDO
C
      DO K=1,KC
       DO L=2,LA
...
C
      DO K=1,KS
       RCDZF=CDZF(K)
       DO L=2,LA
        DU(L,K)=DU(L,K)+RCDZF*DXYIU(L)*(FCAX1(L,K+1)-FCAX1(L,K))
        DV(L,K)=DV(L,K)+RCDZF*DXYIV(L)*(FCAY1(L,K)-FCAY1(L,K+1))
       ENDDO
      ENDDO
C
      DO L=2,LA
       FUHDYE(L)=TVAR3W(L)
     &         +SUB(L)*DELT*DXIU(L)*FCAX1E(L)
       FVHDXE(L)=TVAR3S(L)
     &         -SVB(L)*DELT*DYIV(L)*FCAY1E(L)
      ENDDO
C
C**********************************************************************C
C
...
C
      ...
      FP1(L)=FP1(L)+PCBW(LL,M)*TC+PSBW(LL,M)*TS
      ENDDO
      CET=0.125*DELTD2*G*HRUO(L+1)*HUTMP(L+1)
      IF(ISPBW(LL).GE.1)THEN
       TMP=0.5*DELTD2*SQRT(G*HMU(L+1))*DXIU(L+1)
       CC(L)=CET*(1.+TMP)/TMP
       CE(L)=-CET
       FP1(L)=CET*(2.*FP1(L)
     & -SQRT(G*HMU(L+1))*FUHDYE(L+1)*DYIU(L+1)*HUI(L+1))/TMP
...
C
      ...
      FP1(L)=FP1(L)+PCBE(LL,M)*TC+PSBE(LL,M)*TS
      ENDDO
      CWT=0.125*DELTD2*G*HRUO(L  )*HUTMP(L  )
      IF(ISPBE(LL).GE.1)THEN
       TMP=0.5*DELTD2*SQRT(G*HMU(L))*DXIU(L)
       CC(L)=CWT*(1.+TMP)/TMP
       CW(L)=-CWT
       FP1(L)=CWT*(2.*FP1(L)
     & +SQRT(G*HMU(L))*FUHDYE(L)*DYIU(L)*HUI(L))/TMP
...
C
      ...
      FP1(L)=FP1(L)+PCBS(LL,M)*TC+PSBS(LL,M)*TS
      ENDDO
      CNT=0.125*DELTD2*G*HRVO(LN )*HVTMP(LN )
      IF(ISPBS(LL).GE.1)THEN
       TMP=0.5*DELTD2*SQRT(G*HMV(LN))*DYIV(LN)
       CC(L)=CNT*(1.+TMP)/TMP
       CN(L)=-CNT
       FP1(L)=CNT*(2.*FP1(L)
     & -SQRT(G*HMV(LN))*FVHDXE(LN)*DXIV(LN)*HVI(LN))/TMP
...
C
      ...
      FP1(L)=FP1(L)+PCBN(LL,M)*TC+PSBN(LL,M)*TS
      ENDDO
      CST=0.125*DELTD2*G*HRVO(L  )*HVTMP(L  )
      IF(ISPBN(LL).GE.1)THEN
       TMP=0.5*DELTD2*SQRT(G*HMV(L))*DYIV(L)
       CC(L)=CST*(1.+TMP)/TMP
       CS(L)=-CST
       FP1(L)=CST*(1.*FP1(L)
     & +SQRT(G*HMV(L))*FVHDXE(L)*DXIV(L)*HVI(L))/TMP
...
C
      ...
      LN=LNC(L)
      FP1(L)=FP1(L)+SPB(L)*( DELTI*DXYP(L)*P1(L)
     &      -0.25*G*(UHDY2E(L+1)-UHDY2E(L)
     &              +VHDX2E(LN )-VHDX2E(L)
     &              +2.*UHDY1E(L+1)-2.*UHDY1E(L)
     &              +2.*VHDX1E(LN )-2.*VHDX1E(L)) )
      ENDDO
C
C**********************************************************************C
C
C **  SET NEW TIME LEVEL TERMS IN CONTINUITY EQUATION 
C
C----------------------------------------------------------------------C
C
      DO L=2,LA
      LN=LNC(L)
      FP(L)=FP1(L)-0.25*G*SPB(L)*
     &      ( SUB(L+1)*FUHDYE(L+1)-SUB(L)*FUHDYE(L)
     &       +SVB(LN )*FVHDXE(LN )-SVB(L)*FVHDXE(L)
     &       -4.0*QSUME(L) )
CC      P(L)=0.
...
C**********************************************************************C
C
C **  REVISE UHEX AND VHEX AND TOTAL DEPTHS AT TIME LEVEL (N+1)
C **  HRU=SUB*DYU/DXU & HRV=SVB*DXV/DYV 
C
C----------------------------------------------------------------------C
C
...
C
        ...
        LN=LNC(L)
        HPPTMP=H2P(L)+DELT*DXYIP(L)*(QSUME(L)
     &       -0.25*(UHDYE(L+1)+2.*UHDY1E(L+1)+UHDY2E(L+1)
     &             -UHDYE(L)-2.*UHDY1E(L)-UHDY1E(L)
     &             +VHDXE(LN)+2.*VHDX1E(LN)+VHDX2E(LN)
     &             -VHDXE(L)-2.*VHDX1E(L)-VHDX2E(L)))
        IF(ISGWIE.GE.1) HPPTMP=HPPTMP
     &                         -DELT*DXYIP(L)*(RIFTR(L)+EVAPSW(L))
        HP(L)=SPB(L)*HPPTMP+(1.-SPB(L))*(GI*P(L)-BELV(L))
        ENDDO
...
C**********************************************************************C
C
C **  REVISE FINAL UPDATES OF P,HU, AND HV
C
C----------------------------------------------------------------------C
C
      DO L=2,LA
