...
C
       DO L=LF,LL
        ...
        DZWQ(L) = 1.0 / (DZC(K)*GVCSCLP(L)*HP(L))
...
C
          XMRM = WQVO(L,K,IDNOTRVA)*DZC(K)*GVCSCLP(L)*HP(L)
...
C
     ...
     &                           +(2.1407E+3*TVAL2) )
        WQDOS(L) = EXP(RLNSAT2)
        XDOSAT(L,K)=XDOSAT(L,K)+WQDOS(L)*DTWQ*DZC(K)*GVCSCLP(L)*HP(L)
        IF(K.EQ.KC)THEN
C MRM: 06/20/98
...
C
      IF(ISICE.EQ.1)THEN
        DO L=LF,LL
          WQP19(L) = (1.-RICECOVL(L))*WQP19(L)
          WQKRDOS(L) = (1.-RICECOVL(L))*WQKRDOS(L)
        ENDDO
	ENDIF
C
C----------------------------------------------------------------------C
...
C MRM  ADD WET ATMOSPHERIC DEPOSITION:
          ...
          ENDIF
          IF(K.EQ.KGVCP(L)) WQR20(L) = WQR20(L)
...
C DEFINITIONS    ATM DRY DEP       LOADS          VOLUMN
        ...
        XDOPSL(L,K) = XDOPSL(L,K) + WQRR(L)*DTWQ*DZC(K)*GVCSCLP(L)*HP(L)
        XDOALL(L,K) = XDOALL(L,K) + WQRR(L)*DTWQ*DZC(K)*GVCSCLP(L)*HP(L)
C
        IF(K.EQ.KC)THEN
C DEFINITIONS              ATM WET DEP      VOLUMN
...
C MRM ++++ END NEW CODE
     ...
     *  WQTTM*WQVO(L,K,IDNOTRVA)*WQAOCRPM*DTWQO2*DZC(K)*GVCSCLP(L)*HP(L)
           XDOALL(L,K) = XDOALL(L,K) +
     *  WQTTM*WQVO(L,K,IDNOTRVA)*WQAOCRPM*DTWQO2*DZC(K)*GVCSCLP(L)*HP(L)
           XDORRM(L,K) = XDORRM(L,K) - 
     *   XMRM*WQVO(L,K,IDNOTRVA)*WQAOCRRM*DTWQO2*DZC(K)*GVCSCLP(L)*HP(L)
           XDOALL(L,K) = XDOALL(L,K) -
     *   XMRM*WQVO(L,K,IDNOTRVA)*WQAOCRRM*DTWQO2*DZC(K)*GVCSCLP(L)*HP(L)
         ENDIF
C J.S.
...
C
         ...
         XDODEF(L,K) = XDODEF(L,K) + XMRM*DTWQ*DZC(K)*GVCSCLP(L)*HP(L)
         IF(K.EQ.KC)THEN
           XDOKAR(L,K) = XDOKAR(L,K) + 
     *        WQKRDOS(L)*DTWQ*DZC(K)*GVCSCLP(L)*HP(L) +
     *        WQP19(L)*WQVO(L,K,19)*DTWQO2*DZC(K)*GVCSCLP(L)*HP(L)
           XDOALL(L,K) = XDOALL(L,K) + 
     *        WQKRDOS(L)*DTWQ*DZC(K)*GVCSCLP(L)*HP(L) +
     *        WQP19(L)*WQVO(L,K,19)*DTWQO2*DZC(K)*GVCSCLP(L)*HP(L)
         ENDIF
...
C MRM     *     *DZC(K)*GVCSCLP(L)*HP(L)
C MRM         XDOALL(L,K)=XDOALL(L,K) - WQO18(L)*WQVO(L,K,18)*DTWQO2
C MRM     *     *DZC(K)*GVCSCLP(L)*HP(L)
         XDODZ(L,K) = XDODZ(L,K) + DZC(K)*GVCSCLP(L)*HP(L)
C
       ENDDO
C
C----------------------------------------------------------------------C
...
C
	   ...
	   KBWQ=KGVCP(L)
         IMWQZ = IWQZMAP(L,1)
         WQDFBC(L) = SCB(L)*WQWSC(IMWQZ)*WQV(L,KBWQ,1)
         WQDFBD(L) = SCB(L)*WQWSD(IMWQZ)*WQV(L,KBWQ,2)
         WQDFBG(L) = SCB(L)*WQWSG(IMWQZ)*WQV(L,KBWQ,3)
     +             +WQWSM*DZWQ(L)*WQV(L,KBWQ,IDNOTRVA)
         WQDFRC(L) = SCB(L)*WQWSRP(IMWQZ)*WQV(L,KBWQ,4)
         WQDFLC(L) = SCB(L)*WQWSLP(IMWQZ)*WQV(L,KBWQ,5)
         WQDFRP(L) = SCB(L)*WQWSRP(IMWQZ)*WQV(L,KBWQ,7)
         WQDFLP(L) = SCB(L)*WQWSLP(IMWQZ)*WQV(L,KBWQ,8)
         WQDFRN(L) = SCB(L)*WQWSRP(IMWQZ)*WQV(L,KBWQ,11)
         WQDFLN(L) = SCB(L)*WQWSLP(IMWQZ)*WQV(L,KBWQ,12)
         IF(IWQSI.EQ.1) WQDFSI(L) = SCB(L)*WQWSD(IMWQZ)*WQV(L,KBWQ,16)
       ENDDO
       ENDDO
C
        IF(IWQSRP.EQ.1)THEN
     ...
     *        + WQWSS(IMWQZ)*( WQV(L,KBWQ,10)-WQPO4D(L,KBWQ) ) )
            IF(IWQSI.EQ.1) WQDFSI(L) = SCB(L)*( WQDFSI(L)
     *        + WQWSS(IMWQZ)*( WQV(L,KBWQ,17)-WQSAD(L,KBWQ) ) )
          ENDDO
          ENDDO
         ELSE IF(IWQSRP.EQ.2)THEN
          DO ND=1,NDMWQ
          ...
          DO L=LF,LL
	     KBWQ=KGVCP(L)
           WQDFLP(L) = SCB(L)*( WQDFLP(L)+WSEDO(NS)*( WQV(L,KBWQ,10)
     *                         -WQPO4D(L,KBWQ) ) )
           IF(IWQSI.EQ.1) WQDFSI(L) = SCB(L)*( WQDFSI(L)
     *        + WSEDO(NS)*( WQV(L,KBWQ,17)-WQSAD(L,KBWQ) ) )
          ENDDO
          ENDDO
        ENDIF
C
